...
object extract {
  ...
  val populatedSignals = Array("wheel_speed_front_left",
  ...
  def main(args: Array[String]) {
    ...
    // val et = new extractTableperUser(sc_conf, keyspace)
    ...
    //     if (email == "gahr@crew.li" || email=="ben@ben.com") {
    //       for (feature <- populatedSignals) {
    //         et.analyze_measurement(feature, userId)
    //         et.finalize()
    //       }
    //     }
    //   }
    ...
    val je = new joinExtract(sc_conf, keyspace)
    for (user_data <- user_rdd) {
      if (!user_data.isNullAt("userid") && !user_data.isNullAt("email")) {
        ...
        if (email == "gahr@crew.li" || email=="ben@ben.com") {
            je.analyze_measurement(populatedSignals, userId)
        }
      }
    }
