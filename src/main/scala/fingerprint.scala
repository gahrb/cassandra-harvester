...
class fingerprint(sc_conf: SparkConf, keyspace: String) extends java.io.Serializable{
  ...
  def analyze_measurement(measurement: String, userId: UUID): Unit = {
    ...
    val rdd = sc.cassandraTable(keyspace, measurement).select("time","value").where("user="+userId.toString)
    try{
  ...
  def analyzeValues(valueList: RDD[(UUID,Iterable[(DateTime,Double)])],measurement: String, user: String): Unit={
    ...
    for (user <- valueArr){
      ...
      for (dt <- user._2) {
        writer.writeRow(Seq(dt._1.getMillis,dt._2))
  ...
  def valtoCSV(valueList: RDD[CassandraRow], measurement: String, user: String): Unit={
    ...
    try {
      ...
      case typeErr: NumberFormatException => try{
        //l.info("Error while writing:" + typeErr)
